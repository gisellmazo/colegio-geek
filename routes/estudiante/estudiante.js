...
router.get('/ver_mis_notas', async(req, res)=>{
    ...
    try {
        const result = await pool.query('SELECT materias.nombre, notas.nota FROM notas INNER JOIN materias ON materias.id_materia = notas.id_materia WHERE notas.id_estudiante = $1 ', [id_estudiante])
        if(result.rowCount>0){
            return res.json(result.rows)
        }
    }
